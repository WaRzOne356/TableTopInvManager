...
namespace InventorySystem.UI.Dialogs
{
    ...
    public class AddItemDialog : MonoBehaviour
    {
        ...
        void Awake()
        {
            Debug.Log("[additemdialog] in the awake function");
            ...
            Debug.Log("[additemdialog] at the end of the awake function");
        }
        ...
        private async void AddSelectedItem()
        {
            ...
            try
            {
                ...
                if (item != null)
                {
                    ...
                    {
                        Debug.Log($"[AddItemDialog] OnItemSelected has {OnItemSelected.GetInvocationList().Length} subscribers");
                        OnItemSelected?.Invoke(item);
                    }
                    else
                    {
                        Debug.LogError("[AddItemDialog] OnItemSelected has NO subscribers!");
                    }

                    SetSearchStatus($"Added {quantity}x {item.itemName}", true);
                    Debug.Log($"[additemdialog] Added {quantity}x {item.itemName}");
