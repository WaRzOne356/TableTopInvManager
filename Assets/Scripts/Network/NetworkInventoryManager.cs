...
public class NetworkInventoryManager : NetworkBehaviour
{
    ...
    [Header("Permissions")]
    [SerializeField] private NetworkUserPermission defaultUserPermission = NetworkUserPermission.EditItems;
    ...
    public void AddItemServerRpc(NetworkInventoryItem item, ulong requestingClientId = 0, ServerRpcParams serverRpcParams = default)
    {
        if (!IsServer) return;
        ...
        //check permissions
        if (!HasPermission(clientId, NetworkUserPermission.AddItems))
        {
    ...
    [ServerRpc(RequireOwnership =false)]
    public void UpdateItemQuantityServerRpc(FixedString64Bytes itemID, int newQuantity, ServerRpcParams serverRpcParams = default)
    {
        if (!IsServer) return;

        var clientId = serverRpcParams.Receive.SenderClientId;

        if(!HasPermission(clientId, NetworkUserPermission.EditItems))
        {
    ...
    [ServerRpc(RequireOwnership =false)]
    public void RemoveItemServerRpc(FixedString64Bytes itemId, ServerRpcParams serverRpcParams = default)
    {
        if (!IsServer) return;

        var clientId = serverRpcParams.Receive.SenderClientId;

        if (!HasPermission(clientId, NetworkUserPermission.DeleteItems))
        {
    ...
    [ServerRpc(RequireOwnership =false)]
    public void RequestFullInventorySyncServerRpc(ServerRpcParams serverRpcParams = default)
    {
        if (!IsServer) return;
    ...
    [ServerRpc(RequireOwnership = false)]
    public void SetUserPermissionServerRpc(ulong targetClientId, NetworkUserPermission newPermission, ServerRpcParams serverRpcParams=default)
    {
        if (!IsServer) return;

        var clientId = serverRpcParams.Receive.SenderClientId;

        if (!HasPermission(clientId, NetworkUserPermission.Admin))
        {
    ...
    // HELPER METHODS
    //=======================================================================================================
    private bool HasPermission(ulong clientId, NetworkUserPermission networkUserPermission)
    {
    ...
    public void OnClientConnected(ulong clientId)
    {
        if (IsServer)
        {
            var newUser = new NetworkUserInfo
            {
                ...
                permission = autoPromoteFirstClient && connectedUsers.Count == 0 ? NetworkUserPermission.Admin : defaultUserPermission,
